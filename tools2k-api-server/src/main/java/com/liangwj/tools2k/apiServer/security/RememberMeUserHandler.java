...
import com.liangwj.tools2k.utils.other.BitUtils;
import com.liangwj.tools2k.utils.other.EncryptUtil;
import com.liangwj.tools2k.utils.web.CookieUtils;
...
public class RememberMeUserHandler {
	...
	@PostConstruct
	protected void init() {
		final Map<String, IWebUserProvider> map = this.applicationContext.getBeansOfType(IWebUserProvider.class);

		for (final IWebUserProvider userProvider : map.values()) {
		...
		if (!this.userProviderMap.containsKey(CommonAdminWebUser.class)) {
			...
			log.info("初始化开发后台的管理账号:\n\t账号在配置文件中, 账号:{} 密码:{}", this.prop.getAdmin().getAccount(),
					this.prop.getAdmin().getPassword());
		} else {
			log.info("初始化开发后台的管理账号:\n\t账号由其他服务管理");
	...
	private <T extends IWebUser> T check(Class<? extends IWebUser> clazz, IWebUser user, CookieValue value) {
		...
		final long time = value.expireTime - System.currentTimeMillis();
		...
		final String sign = this.makeTokenSignature(value.expireTime, value.account, user.getEncryptedPassword());
	...
	private String extractRememberMeCookie(HttpServletRequest request, Class<? extends IWebUser> clazz) {
		...
		final String cookieName = this.prop.getRememberMe().getCookieName(clazz);
	...
	public <T extends IWebUser> T getUser(HttpServletRequest request, HttpServletResponse response,
		...
		final String rememberMeCookieString = this.extractRememberMeCookie(request, clazz);
		...
		final CookieValue value = decodeCookie(rememberMeCookieString);

		if (value != null) {
			// 根据用户class寻找这个用户类型的数据提供者
			final IWebUserProvider<T> userProvider = this.getUserProvider(clazz);
			if (userProvider != null) {
				// 如果能找到
				final T tmpUser = userProvider.loadUserByAccount(value.account);
	...
	public void onLoginSuccess(HttpServletRequest request, HttpServletResponse response, IWebUser user) {
		...
		final int maxAge = this.prop.getRememberMe().getCookieAge();
		final String cookieName = this.prop.getRememberMe().getCookieName(user.getClass());

		// 合成要保存到cookie的内容
		final CookieValue value = new CookieValue();
		...
		final String cookieValue = encodeCookie(value);

		CookieUtils.addEncryptCookie(response, "/", cookieName, cookieValue, "", maxAge);

		if (log.isDebugEnabled()) {
			final Date expire = new Date(value.expireTime);
	...
	protected String encodeCookie(CookieValue value) {
		...
		final byte[] byteAry = str.getBytes();
	...
	private String makeTokenSignature(long tokenExpireTime, String username, String password) {
		...
		final String data = username + DELIMITER + tokenExpireTime + DELIMITER + password + DELIMITER
	...
	protected CookieValue decodeCookie(String cookieValue) {
		...
		final byte[] byteAry = EncryptUtil.hexToByte(cookieValue);

		// 左移动4位
		BitUtils.shiftLeft(byteAry, 4);

		final String cookieAsPlainText = new String(byteAry);

		final String[] tokens = StringUtils.delimitedListToStringArray(cookieAsPlainText, DELIMITER);
	...
	public void addUserProvider(IWebUserProvider<?> userProvider) {
		...
		final Class<? extends IWebUser> clazz = userProvider.getSupportUserClassNames();

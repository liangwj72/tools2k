package com.liangwj.tools2k.utils.classUtils;
...
import com.liangwj.tools2k.annotation.form.AFormSetter;
...
public class MethodUtil {
	...
	private final IMethodFilter<MethodInfoOfSetter> setterFilter = new IMethodFilter<MethodInfoOfSetter>() {
		...
		@Override
		public String getPropName(Method method) {
			AFormSetter setterAnno = method.getAnnotation(AFormSetter.class);
			if (setterAnno != null && StringUtils.hasText(setterAnno.propName())) {
				// 如果有注解重定义了名字，就用注解定义的名字
				return setterAnno.propName();
			} else {
				String name = method.getName(); // 名字

...
public class PasswordEncoder {
	 ...
	 * 和表单中的密码进行校验
	 * 
	 * @param passwordInForm  输入的密码
	 * @param passwordEncoded 加密后的密码
	 * @throws InvalidPasswordException 如果密码不对就抛错
	 */
	public static void checkPassword(String passwordInForm, String passwordEncoded, boolean encodePsw)
			throws InvalidPasswordException {
		if (StringUtils.hasText(passwordEncoded) && StringUtils.hasText(passwordInForm)) {
			// 如果两个密码都不为空
			String expected = passwordInForm;
			if (encodePsw) {
				expected = encodePassword(passwordInForm);
			}
	...
	public static void checkPassword(String passwordInForm, String passwordEncoded) throws InvalidPasswordException {
		checkPassword(passwordInForm, passwordEncoded, true);
	}
}
